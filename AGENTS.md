...
25. **Автопродление подписки.** Включается рубильником `AUTORENEW_ENABLED` **вместе** с
    ...
    **недоступности** кассы (транспорт и 5xx, `isYooKassaOutage`); 4xx — это ответ кассы, и в счётчик
    аномалии он не идёт. Админ может **выключить** чужое автопродление, но не включить, и кнопка
