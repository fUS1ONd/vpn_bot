...
- **`internal/database/payments.go`** — таблица payments (включая `provider_paid_at` — момент списания по данным провайдера) и логика подтверждения/ретраев платежей, выборки платежей для сверки
...
- **`internal/bot/pending_reconcile.go`** — сверка зависших платежей с API провайдера: таймер первой сверки, шаг планировщика, сверка платежей человека перед отключением и киком
- **`internal/bot/scheduler.go`** — scheduler подписок и платежей: каждые 30 минут + первый проход при старте; первым шагом сверяет зависшие платежи с провайдером
...
# Автопродление подписки (опционально, по умолчанию выключено)
AUTORENEW_ENABLED=false  # рубильник всей фичи; автосписание возможно только через ЮKassa,
                         # поэтому без YOOKASSA_SHOP_ID и YOOKASSA_SECRET_KEY флаг ничего не включает
...
5. **Платёжный flow:** callback обрабатывается быстро, долгие retry не держат HTTP-запрос
   ...
   **Переиспользуемый pending закрывать нельзя.** В `createPaymentForProvider`
   ...
   Исключение одно: запись старше `pendingMaxAge` (суток) не переиспользуется — предел жизни
   платежа считается от её создания, и выданная по ней ссылка закрылась бы ближайшей сверкой,
   пока человек платит; такая запись закрывается, а платёж создаётся заново.
...
6. **Плановый scheduler:** стартует сразу при запуске и далее работает каждые 30 минут; **первым** шагом сверяет зависшие платежи с провайдером (см. пункт 24), дальше обрабатывает confirmed_not_activated, уведомления, disable и grace kick, а **последним** шагом добивает непробитые чеки «Моего налога». Шаг чеков объявлен через `defer` в начале прохода, но выполняется в конце: поход в ФНС долгий, и уведомления с автокиками ждать его не должны, а `defer` нужен потому, что шаги между ним и концом функции умеют выходить раньше по ошибке Remnawave — от неё чеки не зависят
...
21. **Автопробитие чеков в «Мой налог»** — включается наличием `MOYNALOG_INN` и
    ...
    в `docker-compose.yml` обязательны, иначе `lknpd.nalog.ru` не резолвится. **Дата дохода —
    момент списания**, а не момент, когда бот узнал об оплате: чек датируется
    `payments.provider_paid_at` (ЮKassa `captured_at`), и только при его отсутствии —
    `confirmed_at`. На стыке месяцев это разные налоговые периоды, а платёж, подхваченный
    сверкой, бот принимает позже списания. Подробности и обоснования —
    `docs/specs/2026-08-06-moynalog-receipts-design.md`,
    `docs/adr/0001-own-moynalog-client.md` и `docs/adr/0003-pending-payment-reconcile.md`.
...
24. **Сверка зависших платежей с провайдером** (`internal/bot/pending_reconcile.go`) — бот
    спрашивает провайдера сам, а не ждёт уведомления. Поводом стал платёж №135: деньги
    списаны, вебхук не дошёл, кнопку «Проверить оплату» человек не нажал — платёж навсегда
    остался pending, подписка не продлилась, чек не пробит. Сроки: первая сверка —
    `pendingFirstCheckDelay` (15 минут) таймером в памяти (`schedulePendingPaymentCheck`;
    таймер живёт только в памяти процесса, потерянный при перезапуске платёж подхватит
    планировщик), дальше — шаг `reconcilePendingPayments` в каждом проходе планировщика
    (раз в 30 минут и при старте), предел жизни платежа — `pendingMaxAge` (сутки). Инварианты: оплаченный платёж
    принимается **тем же путём**, что и по вебхуку (`handleConfirmedFromProviderState` после
    `verifyProviderPayment`) — подписка, чек, сообщение человеку; отменённый отменяется,
    chargeback обрабатывается как в callback. **Закрытие только после последней сверки:**
    платёж без конечного статуса становится `expired` только по истечении суток и только
    после вопроса провайдеру (ответ «не оплачен» или отсутствие ответа к сроку); срок жизни
    ссылки провайдера пределом не служит — слепого закрытия по `expires_at` больше нет,
    `ExpireOldPendingPayments` удалён и из шага планировщика, и из кнопки «Проверить оплату».
    Локально закрытые (`expired`/`canceled`) платежи сверяются те же сутки: бот закрывает
    платёж сам при смене способа оплаты, а деньги по нему могли пройти. Перед отключением и
    автокиком (`processTrialUser`, `processPaidUser`) ждущие оплаты платежи человека
    сверяются независимо от возраста (`reconcileUserPaymentsBeforeKick`): кик необратим, а платёж может быть
    моложе 15 минут. Шаг планировщика ограничен `defaultReconcilePassBudget` (5 минут),
    держит свою панику при себе и проверяет остановку бота — за ним стоят уведомления,
    отключения и автокики. Подтверждение сверкой пишется в лог `slog.Warn` («Платёж
    подтверждён сверкой: уведомление провайдера не дошло»); метрик и сообщений админу нет
    намеренно. Обоснования — `docs/adr/0003-pending-payment-reconcile.md`.
25. **Автопродление подписки.** Включается рубильником `AUTORENEW_ENABLED` **вместе** с
    настроенной ЮKassa (`config.AutorenewingEnabled()`), по умолчанию выключено: автосписание
    возможно только через кассу, Platega здесь — крипто-приём, а по криптоплатежу не списать.
    Один предикат `b.autorenewAvailable()` гасит всё сразу: абзац согласия, `save_payment_method`,
    кнопки и шаг scheduler. Инварианты: **согласие (Автопродление) и Способ автосписания — две
    сущности** (`internal/database/autorenewals.go`), связывать их `CHECK` нельзя — согласие без
    Способа и Способ без согласия оба нормальны; пропавший Способ (`methodGoneReasons`,
    `internal/yookassa/client.go`) гасит **Способ**, а не согласие, и при следующей ручной оплате
    картой автопродление оживает само. Попытка **записывается до** обращения к кассе
    (`RecordAutorenewAttempt`, ключ `(telegram_id, expire_at, attempt_no)`) — тем же приёмом, что
    и `ClaimReceipt`: иначе падение между запросом и записью дало бы вторую попытку с новым ключом
    идемпотентности и списание дважды. Попытки привязаны к `expireAt`, а не к календарю: сдвинулся
    `expireAt` — новый цикл; выключение и повторное включение попытки **не** обнуляет. Списания —
    отдельный шаг прохода (`runAutorenewCharges`, пул 5), идущий **до** ветки disable и целиком под
    `getPaymentMutex(telegramID)`: гонка «scheduler списывает, человек в ту же секунду платит
    руками» — самый дорогой баг этой фичи. В режиме обслуживания не списываем, и попытка при этом
    **не** расходуется; не расходуется она и когда деньги за цикл уже приняты
    (`HasConfirmedPaymentSince`, включая `confirmed_not_activated` — при упавшей активации
    `expireAt` стоит на месте, и T−0 списал бы второй раз за месяц) или когда у пользователя открыта
    живая ссылка на ручную оплату (`redirect_url` у незакрытого платежа): деньги по ней уходят в
    кассе, а не в нашем процессе. Подтверждение идёт через `handleConfirmedSilently` с
    `providerVerified = true` (п. 5): автосписание рождается из сверенного ответа API кассы, а не из
    вебхука. `provider_payment_id` сохраняется **сразу после ответа кассы, при любом исходе**
    (`SetProviderPaymentDetails`): вебхук ищет платёж только по нему, и без этой записи дозревший до
    `succeeded` pending не нашёлся бы вовсе — деньги приняты, подписка не выдана. У записи платежа
    автосписания обязателен `expires_at` (`autorenewPaymentTTL` = 1 час): `GetPendingPayment` считает
    pending без срока живым, и запись висела бы у пользователя незакрытым платежом; закрывает её
    сверка зависших (п. 24) — по ответу кассы или через сутки. Отмену автосписания, найденную
    сверкой или вебхуком, `handleCanceled` записывает молча — без «попробуйте снова».
    **Неизвестный исход, при котором касса не назвала свой платёж, даёт следующей попытке цикла тот
    же ключ идемпотентности** (`UnresolvedAutorenewAttempt` + переиспользование записи, её оживляют
    из `expired`): обращение могло дойти и деньги могли уйти, новый ключ означал бы второе списание
    за месяц. Требовать при этом статус `pending` нельзя — между попытками сутки, запись уже помечена
    `expired`; сумма обязана совпадать, иначе касса откажет на других параметрах и идём новой записью.
    **Новый ключ в цикле — только после определённого отказа кассы** (`autorenewCycleUnsettled`):
    если касса назвала свой платёж и он не отменён (pending, ответ не сошёлся, подтверждение
    упало), вторая попытка не делается — его судьбу решают вебхук и сверка; если не назвала,
    повтор по прежнему ключу допустим только `autorenewKeyLifetime` = 23 ч (ЮKassa помнит ключ
    сутки), дальше — пропуск и алерт владельцу. Попытки разнесены почти ровно на сутки, поэтому
    повтор идёт **в окне своей же попытки** каждым проходом (`autorenewResendable`), а не
    следующей попыткой; в счётчик алерта о недоступности кассы повторы не идут.
    **Способ сохраняется только из `succeeded` и только по платежу, созданному после последнего
    гашения Способа** (`method_cleared_at`, `rememberAutorenewMethod`): в `rememberAutorenewMethod`
    приходит каждый сверенный ответ — повторный вебхук, сверка отменённых, — и без этих фильтров
    мёртвая карта возвращалась бы сверкой отказа `card_expired`, а отвязанная — ответом по старому
    платежу. Выключение согласия (`disableAutorenew`) идёт под `getPaymentMutex`, как и отвязка, а
    сообщения пользователю — и об успехе, и об отказе — отправляются вне мьютекса.
    «Деньги за цикл уже приняты» включает любой висящий `confirmed_not_activated`
    (`HasUnactivatedPayment`), а не только принятые за последние сутки.
    **Ручная оплата запись автосписания не переиспользует** (`IsAutorenewPayment` в
    `createPaymentForProvider`, п. 5): её ключ выписан под запрос с `payment_method_id` и без
    `confirmation`, и по нему касса отказала бы обычному платежу — человек не смог бы оплатить вовсе.
    Граница второй попытки (T−0) — **две** проверки сразу: статус `ACTIVE` в панели (отсекает
    отключённых, в том числе руками владельца) **и** отсутствие нашей пометки `expired` (без неё окно
    T−0 держалось бы все 72 часа grace: статус меняем мы, а не панель). Осознанное следствие: на
    панелях, которые сами метят пользователя `EXPIRED` по своему расписанию, вторая попытка не
    сработает — это принятое решение, списать с отключённого хуже, чем потерять попытку. Подавление
    `expire_3d`/`expire_1d` **условное** (`autorenewSuppressesExpiryNotice`): молчим, только пока
    автопродление ещё обещает сработать, а после любой неуспешной попытки цикла
    (`HasFailedAutorenewAttempt`) предупреждения возвращаются — иначе человек дошёл бы до отключения
    без единого сообщения. Провал по пропавшему Способу получает **отдельный** текст: второй попытки
    у него не будет, и «следующая попытка завтра» заставила бы класть деньги на мёртвую карту.
    Платёж автосписания, дожатый плановым retry активации, получает текст автопродления, а не
    «Оплата прошла!» (`paymentConfirmationMessage` через `IsAutorenewPayment`): «я не платил» на
    сообщение о списанных деньгах — прямая дорога к chargeback. Алерт владельцу поднимается только по
    **недоступности** кассы (транспорт, 5xx, 401 — сломанный ключ, 403 — отозвано разрешение на
    автоплатежи; `isYooKassaOutage`); остальные 4xx — ответ по конкретному платежу, в счётчик
    аномалии они не идут. Админ может **выключить** чужое автопродление, но не включить, и кнопка
    выключения (как и на экране пользователя) привязана к **живому согласию**, а не к состоянию
    «включено»: у пользователя с пропавшим Способом или истёкшей подпиской карточка показывает
    «недоступно», но выключить согласие он вправе. **Отвязка Способа — своя точка входа**
    (`internal/bot/payment_method.go`, кнопка «💳 Способ оплаты» в карточке по условию непустого
    `methodTitle`), а не пункт внутри автопродления: ЮKassa согласовывает рекурренты только при
    интерфейсе, где покупатель отвязывает способ сам, без поддержки, и такой интерфейс обязан
    называться своим именем. Прежняя кнопка «Выключить» требованию не отвечала — гасила согласие,
    оставляя `payment_method_id` в базе. Отвязка гасит **обе** сущности: согласие, пережившее
    отвязку, обещало бы списания, которых нечем сделать. «Выключить» же Способ намеренно
    оставляет — включить обратно можно сразу; разная цена ошибки и даёт подтверждение у отвязки
    и один тап у выключения. Sticky-флага отказа нет намеренно: абзац согласия показывается при
    **каждой** оплате, поэтому следующая ручная оплата картой сохранит Способ заново по свежему
    согласию, а не молча, — автопродление при этом остаётся выключенным до явного включения.
    Пользовательский flow и тексты —
    `docs/behavior/autorenewal.md`, разделение сущностей и выбор `save_payment_method` —
    `docs/adr/0004-autorenewal-consent-and-method.md`.
    `period_months` в `payments` и `autorenewals` сегодня **всегда 1**: мультимесяц в объём не
    входит, ветвлений под другие значения быть не должно.
...
- **Сверка зависших платежей**: `docs/adr/0003-pending-payment-reconcile.md` — почему бот спрашивает провайдера сам, откуда сроки 15 минут / 30 минут / сутки и почему чек датируется списанием
- **Автопродление: согласие и Способ**: `docs/adr/0004-autorenewal-consent-and-method.md` — почему согласие и Способ автосписания разведены на две сущности и почему Способ берётся из обычной оплаты, а не отдельной привязкой
- **Автопродление подписки**: `docs/behavior/autorenewal.md` — абзац согласия, состояния карточки, экран автопродления, сообщения об автосписании и админская часть
