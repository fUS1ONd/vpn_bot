...
25. **Автопродление подписки.** Включается рубильником `AUTORENEW_ENABLED` **вместе** с
    ...
    **не** расходуется; не расходуется она и когда деньги за цикл уже приняты
    ...
    автосписания обязателен `expires_at` (`autorenewPaymentTTL` = 1 час): `ExpireOldPendingPayments`
    сравнивает даты и `NULL` пропускает, так что без срока pending висел бы у пользователя вечно.
