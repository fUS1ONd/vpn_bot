...
Выберите действие:`
	...
	MsgAccountCreating = `⏳ Создаю аккаунт…`
...
<code>%s</code>`
	...
	// MsgRateLimitedCallback — всплывашка на нажатие inline-кнопки при превышении
	// лимита. Показывается поверх кнопки, в чат не уходит, поэтому короткая.
	MsgRateLimitedCallback = "Слишком быстро, секунду"

	// MsgRateLimitedInline — надпись на кнопке над пустой выдачей в выборе чата.
	// Место там на одну строку, и объяснить надо ровно одно: дело не в поломке.
	MsgRateLimitedInline = "Слишком быстро, секунду"

	// MsgRateLimitedText — единственное на жизнь бакета объяснение для тех, кто
	// спамит текстом: молчащий бот выглядит сломанным.
	MsgRateLimitedText = `<b>⏳ Слишком быстро</b>

Подождите секунду и повторите — часть сообщений пропущена.`
