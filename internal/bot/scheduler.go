...
func (b *Bot) runSubscriptionSchedulerPass() {
	...
	// 3. Списания по включённому Автопродлению — отдельным шагом и до основного
	// цикла. До: попытка T−0 обязана отработать раньше ветки disable, а список
	// пользователей панели загружается ниже — успевшие продлиться попадут в
	// цикл уже с новым expireAt, и обрабатывать их как должников не придётся.
...
func (b *Bot) processPaidUser(telegramID int64, ref remnawave.UserRef, expireAt, now time.Time) {
	// Пользователю с включённым автопродлением про скорое окончание не пишем:
	// он специально включил автопродление, чтобы об этом не думать, и для него
	// это ложная тревога. Окно за сутки у него занято попыткой списания.
