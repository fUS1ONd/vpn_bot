...
func TestAddModerator(t *testing.T) {
	...
	_, err := db.CreateUser(100, "testmod", "Тест", strPtrTest("uuid-mod-1"), nil, nil, nil)
	require.NoError(t, err)
...
func TestIsModerator(t *testing.T) {
	...
	_, err := db.CreateUser(200, "user200", "Юзер", strPtrTest("uuid-200"), nil, nil, nil)
	require.NoError(t, err)
...
func TestGetModerator(t *testing.T) {
	...
	_, err := db.CreateUser(300, "mod300", "Мод", strPtrTest("uuid-300"), nil, nil, nil)
	require.NoError(t, err)
...
func TestGetAllModerators(t *testing.T) {
	...
	t.Run("Несколько модераторов", func(t *testing.T) {
		_, err := db.CreateUser(400, "mod1", "Первый", strPtrTest("uuid-400"), nil, nil, nil)
		require.NoError(t, err)
		_, err = db.CreateUser(401, "mod2", "Второй", strPtrTest("uuid-401"), nil, nil, nil)
		require.NoError(t, err)
...
func TestRemoveModerator(t *testing.T) {
	...
	_, err := db.CreateUser(500, "mod500", "Мод", strPtrTest("uuid-500"), nil, nil, nil)
	require.NoError(t, err)
