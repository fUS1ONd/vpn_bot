...
func TestFirstReferralInviterSurvivesReentry(t *testing.T) {
	...
	_, err = db.CreateUserWithInviter(500, "guest", "Guest", "uuid-first", first.SubscriptionPrice, nil, &first.CreatedBy)
	require.NoError(t, err)
	...
	_, err = db.CreateUserWithInviter(500, "guest2", "Guest", "uuid-second", second.SubscriptionPrice, nil, inviter)
	require.NoError(t, err)
