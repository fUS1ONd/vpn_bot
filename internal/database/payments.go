...
func (db *DB) GetPaymentByID(id int64) (*Payment, error) {
	...
	err := db.conn.QueryRow(
		`SELECT id, telegram_id, moderator_id, amount, payment_method, status, platega_transaction_id, provider, provider_payment_id, provider_request_key, provider_fee_percent, is_test, period_months, redirect_url, expires_at, created_at, confirmed_at, provider_paid_at
		 FROM payments WHERE id = ?`, id,
	).Scan(&p.ID, &p.TelegramID, &modID, &p.Amount, &p.PaymentMethod, &p.Status, &txID, &p.Provider, &providerPaymentID, &providerRequestKey, &providerFeePercent, &isTest, &p.PeriodMonths, &redirectURL, &expiresAt, &p.CreatedAt, &confirmedAt, &providerPaidAt)
