...
Telegram бот для управления VPN-клиентами через 3X-UI панели с поддержкой двух серверов (Россия и Европа). Написан на Go для максимальной производительности и простоты развертывания.
...
- Docker
- Go 1.22+ (для локальной разработки) Compose
...
- **Telegram Bot**: `telebot` (gopkg.in/telebot.v3) - для работы с Telegram API
- **SQLite**: `go-sqlite3` - база данных пользователей
- **HTTP**: стандартная библиотека `net/http` - для 3X-UI API и сервера подписок
- **Конфигурация**: `godotenv` - загрузка .env файлов
