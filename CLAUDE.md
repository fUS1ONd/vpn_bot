...
- **`internal/bot/referral_share.go`** — кнопка «📤 Поделиться»: inline-ответ на выбор чата и текст приглашения для чужого чата
...
остановит прод. Цели `make stand-*` подставляют флаги сами.

Полностью — `docs/testing-stand.md`.
...
18. **Общие приглашения** — referral-ссылки создают только пользователи с действующей
    ...
    Кнопка «📤 Поделиться» (`internal/bot/referral_share.go`) открывает нативный выбор
    чата через `switch_inline_query` и требует включённого inline-режима бота
    (`/setinline` у @BotFather) — без него запрос до бота не дойдёт. Инварианты: ответ на
    inline-запрос обязателен в любом исходе (иначе крутилка в поле ввода не разрешится),
    `canAccessReferralSection` проверяется и здесь, иначе inline стал бы обходом бана, а
    текст для чужого чата самодостаточен и не содержит предупреждений владельцу.
...
23. **Канал сообщества.** Форум-супергруппа, включается наличием **обеих** переменных
    ...
    `internal/database/community.go` (таблица `community_members`). Инвариант приписок:
    участником Канала считается не только тот, чью заявку одобрил бот, но и любой, кого
    подтвердил живой `getChatMember` (`alreadyInCommunity` через шов `b.chatMemberOf`,
    объявлен в `internal/bot/handlers.go`) — вступившие по прямой ссылке владельца и до
    появления гейта иначе годами звались туда, где давно сидят. Порядок шагов в
    `claimCommunityMention` задан ценой каждого: фича → «Платящий» → занять недельный
    кулдаун (`claimMentionCooldown`) → и только потом сеть; кулдаун занимается **до**
    похода в Telegram, чтобы сетевой вызов остался вне `communityMentionMu`, а участнику
    эта неделя ничего не стоит — пометка ставится тут же и гасит приписку навсегда.
    Проверка fail-open: ошибка API, разжалованный бот и ненастроенный шов — «снаружи»,
    только `slog.Warn` без алерта владельцу (цена — один лишний абзац в неделю). Домен,
    инварианты и обоснования решений — в
...
- **Общие приглашения**: `docs/behavior/referral-invites.md` — кто может создавать ссылки, лимиты, отзыв, кнопка «Поделиться» и архивные moderator-поля
- **Тестовый стенд**: `docs/testing-stand.md` — запуск, изоляция от прода, пульт заглушки (срок, статус, трафик, устройства, отказы, задержки), чего на стенде нет
